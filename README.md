...
| `STRIPE_MEMBERSHIP_PRICE_ID_ANNUAL` | for the yearly price | Optional, and the only Stripe variable that is. Missing means the yearly button is not rendered and `/api/stripe/checkout` refuses `plan: "annual"` — membership still sells monthly. Set it and the page offers both. |
